...
# the player is checked based on it's unique id
    ...
    while True:
        try:
            ...
            if include_player_info and 'included' in resp:
                ...
                data = data.merge(player_info, how='left', left_on='relationships.new_player.data.id',
                                  right_on='id', suffixes=('', '_player'))
...
def fetch_and_process_data():
    ...
    # in the case when a constant update is needed
    # Flag to track if any new players are processed
    # has_updates = False
    ...
    for _, row in data.iterrows():
        ...
        if is_new_player(player_id):
            ...
            try:
                start_time = row.get('attributes.start_time', 'N/A')
                print(f"Raw start_time value: {start_time}")  # Debugging log

                if start_time != 'N/A':
                    # Parse the ISO 8601 datetime, which is timezone-aware
                ...
                # Debugging log
                print(f"Raw start_time causing error: {start_time}")
                start_time_formatted = "N/A"

            # formatting updated at time
            try:
                updated_at_time = row.get('attributes.updated_at', 'N/A')
                # Debugging log
                print(f"Raw start_time value: {updated_at_time}")
                ...
                print(f"Error formatting start time: {e}")
                # Debugging log
                print(f"Raw start_time causing error: {updated_at_time}")
                updated_at_time_formatted = "N/A"
    ...
    # in the case when a constant update is needed
    # # Log if no updates were processed
    #     if not has_updates:
    #         print("No update yet.")
...
def main():
    ...
    while True:
        print(
            f"Running scraper at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}...")
